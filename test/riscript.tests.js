...
describe(title, function () {
  ...
  describe('Gates', function () {
    ...
    it('Throws on bad gates', function () {
      expect(() => riscript.evaluate('$a=ok\n[ @{$a: ok} hello]', 0)).to.throw(); // unquoted str
      expect(() => riscript.evaluate('[@{} [a|a] || [b|b] ]')).to.throw(); // no ops in gate
    });

    it('Handles transforms in gate operands', function () {
      let ctx, res;
      ctx = { getHours: () => new Date().getHours() };
  ...
  describe('Evaluation', function () {
    ...
    it('Resolves static objects from context', function () {
      ...
      expect(riscript.evaluate('Meet [$person].name',
        { person: { name: 'Lucy' } }))
        .eq('Meet Lucy');
      ...
      expect(riscript.evaluate('Meet [$person=$lucy].name',
        { lucy: { name: 'Lucy' } }))
        .eq('Meet Lucy');
    ...
    it('Resolves statics from context', function () {
      ...
      res = riscript.evaluate('#bar=[$man | $boy]\n$bar:$bar',
        { man: '[BOY|boy]', boy: '[BOY|boy]' }, { trace: 0 });
      expect(res).to.be.oneOf(['BOY:BOY', 'boy:boy']);

      res = riscript.evaluate('[#bar=[man | boy]]:$bar');
      expect(res).to.be.oneOf(['man:man', 'boy:boy']);

      res = riscript.evaluate('[#bar=[$man | $boy]]:$bar', { man: '[MAN|man]', boy: '[BOY|boy]' });//{onepass:1,...T });
